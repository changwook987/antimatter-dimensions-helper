...
export const ECRewards: Reward = {
    "1": {
        ...
        formula:
            "×`max(time in this eternity / 10, 0.9) ^ 0.3 + (completions * 0.05)`",
    },
    "2": {
    ...
    "8": {
        ...
        formula:
            "`max(0, (log10(log10(Infinity Power) + 1) ^ 0.03 * completions) - 1)`%",
    },
    "9": {
...
const SecondaryUnlocks: Unlock = {
    ...
    "7": (completion: number): string =>
        `1e${
            new Decimal("1e500000").times(
                new Decimal("1e300000").pow(completion)
            ).exponent
        }`,
    "8": (completion: number): string =>
        `1e${
            new Decimal("1e4000").times(new Decimal("1e1000").pow(completion))
                .exponent
        }`,
    "9": (completion: number): string =>
        `1e${
            new Decimal("1e17500").times(new Decimal("1e2000").pow(completion))
                .exponent
        }`,
    "10": (completion: number): string =>
        `1e${
            new Decimal("1e100").times(new Decimal("1e20").pow(completion))
                .exponent
        }`,
};
...
const ECGoals = {
    "1": (completion: number) =>
        `1e${
            new Decimal("1e1800").times(new Decimal(1e200).pow(completion))
                .exponent
        }`,
};
...
// eslint-disable-next-line max-len
export const orderWithMultSigns = [
    "1×1",
    "2×1",
    "1×2",
    "3×1",
